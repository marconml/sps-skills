# Editorial Performance Report Contract

Read this reference only when the user requests a report, editorial review, management readout, or comparable presentation artifact. The report is a read-only evidence deliverable and does not authorize SPS prompt changes, publishing, scheduling, replies, or moderation.

## Stable Reading Spine

Keep this section order stable across business units and reporting periods. The evidence and optional sub-analyses may change; the main reading logic should not.
...
Derive every suggested hook and visual direction from a named performance mechanism or comparator shown in the evidence. For high performers, prefer `What to replicate` and omit a rewrite unless a supported weakness exists. For lower performers, label the rewrite and visual direction as a test, identify the evidence basis, preserve factual conditions, and do not invent list counts, claims, people or stakes absent from the approved source.

Use the source content's language for editorial examples. Suggested hooks, visual directions and rewrites for Traditional Chinese content should remain in Traditional Chinese even when the report narrative is English.

### Methodology / Caveats

Place methodology and detailed limitations at the end or in a collapsed section. Name `refine-pillar-prompts` and the methods used. Explain matching, performance normalization, ad exclusion, missing data, classification, comment sampling, and causal limits only to the depth needed to audit the findings.

## Evidence Labels

Use the same labels throughout the report:

- **Observation** — directly supported by approved data.
- **Inference** — a reasonable performance explanation tied to named observations and limitations.
- **Hypothesis / Test** — performance-blind, editorial, single-example, or otherwise unverified proposal.

Keep the label next to the claim. Do not make a reader infer whether a recommendation is proven.

## Stable Presentation, Flexible Sub-Analysis

Keep headings, numbering, information hierarchy, evidence-card anatomy, table density, colors, typography and navigation broadly consistent with the prior approved report for that business unit. Add or omit subordinate charts and subsections according to the available signal; do not fill every month with empty or weak analysis merely for symmetry.
